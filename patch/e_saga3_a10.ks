...
;---------------------------------
*map
*E_saga3_A10|空環市　陸日目の対峙
@title name="&tf.title+  '---　空環市　陸日目の対峙'"
@eval exp=" sf.title_list_8_1[3]=1 "
@fobgm
...
;■イゴンくん
@image layer=10 storage="igonkun2.png" visible=true top=430 left=450 page=back
...
;◆---------------------------------------
;2回分回る
@if exp="f.map_count_E_saga3_A10 != 2"
...
;★すすき野原(N)
@locate x=0 y=0
@button enterse="aka_se_switch04" clickse="ak_se_砂嵐_02" graphic="mapicon_machi_n2.png" recthit=false  target="*jump_to_file" exp="tf.jumpTo='E_saga3_A10n.ks'" cond="f.scenario_flg_E_saga3_A10n!=1" hint=""

;★駅(O)

;▼神社入り口(Q)
@locate x=0 y=5
@button enterse="aka_se_switch04" clickse="ak_se_砂嵐_02" graphic="mapicon_machi_q2.png" recthit=false  target="*jump_to_file" exp="tf.jumpTo='E_saga3_A10q.ks'" cond="f.scenario_flg_E_saga3_A10q!=1" hint=""
...
@eval exp="tf.linkfrom='E_saga3_A10.ks',tf.target='*map'"
@else
;呼び出し元
@eval exp="f.playmode='E_saga3_A10.ks'"
