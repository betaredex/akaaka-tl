;‚`‚`‚`ƒVƒiƒŠƒI
;2010/09/26 ‰¼ì¬F‚©‚È‚ñ
;2010/11/20@ƒ†ƒEƒ~
;2010/11/28@Z³Aƒqƒgƒrƒgq‹Ÿ—§‚¿ŠG‘}“üi‚‹´j
;2011/4/6@’²®i‚‹´j
;2011/4/20@ƒ^ƒCƒgƒ‹‘}“üi‚‹´j
;2011/4/20@ƒ^ƒCƒgƒ‹ƒŠƒXƒg‘Î‰i‚©j
;------------------------------------------------------------------------
;Íƒ^ƒCƒgƒ‹‚ğ•\¦

*s0-02-06|—h‚ê‚é“”‚è‚Éá‚¦‚é–é
@title name="&tf.title+  '---@—h‚ê‚é“”‚è‚Éá‚¦‚é–é'"
@fobgm

@resetmsg
@cm
@seopt volume=100
@bgmopt volume=100

@plbgm set="bgmname='AKA_BGM_M24'"


@call target="*BG_Õ‚P" storage="set_bg.ks" 
@trans-n
@wait time=800

@chara3 b="yue_b3_A006" f="yue_f3_a_a_e" o="yue_o3_A001" visible=true
@trans-n
@messagelay
@plse set="sename='yue_A00241'"
@y—Rz
Oh wow, look at all the paper lanterns. How pretty~
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@plse set="sename='krg_A00300'"
@y•ŒÏz
They made these for the Master too. See, they look just like tsubaki flowers. Ainft they amazing?
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_a_e" o="yue_o3_A001"
@trans-n
@plse set="sename='yue_A00242'"
@y—Rz
Yeah, itfs really cool. They look like theyfre filled with red flowers.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_a_d" o="yue_o3_A001"
@trans-n
@plse set="sename='yue_A00243'"
@y—Rz
cYou know, they remind me of Tomorifs lanterns.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_a_d" o="yue_o3_A002"
@trans-n
@plse set="sename='krg_A00301'"
@y•ŒÏz
Thatfs true. His lanterns are supposed to be a symbol of the Masterfs power, or somethinf.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_a_d" o="yue_o3_A001"
@trans-n
@plse set="sename='krg_A00302'"
@y•ŒÏz
Tch, if he didnft have to work, Ifd bet Tomori would have wanted to come with us~
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_c_d" o="yue_o3_A001"
@trans-n
@plse set="sename='yue_A00244'"
@y—Rz
True, but instead, he went so far as to put the lights out for us, so we wouldnft be seen when we went down.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_c_d" o="yue_o3_A003"
@trans-n
@plse set="sename='krg_A00303'"
@y•ŒÏz
Yeah, he might have a real mean face, but deep down, hefs a guy you can count on~ Go Tomori!
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A006" f="yue_f3_d_b_g" o="yue_o3_A003"
@trans-n
@plse set="sename='yue_A00245'"
@y—Rz
Well, he wears a mask, so we donft know what his face looks likec but Ifm sure hefs not ugly or anything.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A006" f="yue_f3_d_b_g" o="yue_o3_A002"
@trans-n
@plse set="sename='krg_A00304'"
@y•ŒÏz
Anyways, we should get him a souvenir! So he keeps helpinf us!
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A006" f="yue_f3_c_b_a" o="yue_o3_A002"
@trans-n
@plse set="sename='yue_A00246'"
@y—Rz
Yeah. That sounds goodc is what Ifd like to say, but arenft we out of money?

@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A006" f="yue_f3_c_b_a" o="yue_o3_A003"
@trans-n
@plse set="sename='krg_A00305'"
@y•ŒÏz
Itfs fine, just take him some air from here!
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A006" f="yue_f3_d_c_g_a" o="yue_o3_A003"
@trans-n
@plse set="sename='yue_A00247'"
@y—Rz
Huh? What kind of souvenir is that?
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A006" f="yue_f3_d_c_g_a" o="yue_o3_A004"
@trans-n
@plse set="sename='krg_A00306'"
@y•ŒÏz
Itfs air filled with delicious smells, whatfs not to love? Come on, fill your lungs with it! In, out! In, out!
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="yue_b3_A002" f="yue_f3_a_a_g_a" o="yue_o3_A004"
@trans-n
@plse set="sename='yue_A00248'"
@y—Rz
cNo, I donft think hefll want thatc
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse


@resetmsg
@chara3 visible=false
@trans-n
@blackout
@wait time=1000

@call target="*BG__Ğ— _–éÁ“”" storage="set_bg.ks" 
@trans-n
@messagelay
@chara3 b="tomo_b1_A001"
@trans-n
@plse set="sename='tmr_A00000'"
@y“”çz
ccccccc
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@plse set="sename='tmr_A00001'"
@y“”çz
ccHmhm. Yue, and Kurogitsune. They should be enjoying themselves right about now.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse

@chara3 b="tomo_b1_A002"
@trans-n
@plse set="sename='tmr_A00002'"
@y“”çz
Every year, the world becomes visible when it is lit up by all the lights in this way. cIt is beautiful.
@ws canskip=true cond="kag.autoMode"
@endmessage
*|
@stopse




@resetmsg
@chara3 visible=false
@trans-n
@whiteout
@wait time=2000



;ƒVƒiƒŠƒI––”ö‚Ìˆ—
*end

@eval exp="f.Õ‚P_’ñ“” = 1"  

;ƒeƒXƒgˆÈŠO‚È‚ç
@if exp="f.playmode!='kobetsu.ks'"
;‚±‚Ìƒtƒ@ƒCƒ‹‚ğ“Ç‚ñ‚¾ƒtƒ‰ƒOi”O‚Ì‚½‚ß‹L˜^j
@eval exp="f.scenario_flg_s0_02_06 = 1"
@eval exp="sf.scenario_flg_s0_02_06 = 1"
;@eval exp="f.aed5 = 1"


;‚±‚ÌƒtƒF[ƒY‚ÌMAP‚ğŒÄ‚Ño‚µ‚½ƒJƒEƒ“ƒg
@eval exp="f.map_count_s0_02_01map  ++"
;MAP‚É–ß‚é
@jump storage="&f.playmode"

;ƒeƒXƒg‚Ìê‡
@else
;ŒÄ‚Ño‚µŒ³‚É–ß‚é
@return
@endif

